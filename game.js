...
(function(window, $) {
    ...
    var socket = io('localhost:9700', { transports: ['websocket'] });
    var nsp2 = "###!!!###";
    ...
    function Init() {
        ...
        window.connect('ws://localhost:443')
    }
        ...
        if (null != g_scorePartitions || 0 != g_scoreEntries.length) {
            if (null != g_scorePartitions || g_showNames) {
                ...
                if ($('#nick').val() != null && showMap) {
                    ...
                    if (g_playerCellDestroyed === false && spectateMode === false) {
                        t = Date.now()
                        if(t - tickMapSent > 1000){
                            socket.emit('coords', blobData)
                            tickMapSent = t
                        }
                    }
            ...
            if (-1 != items.indexOf('Android')) {
                ...
                if ('undefined' == typeof console || 'undefined' == typeof DataView || 'undefined' == typeof WebSocket || null == canvasTest || null == canvasTest.getContext || null == window.localStorage) {
                    ...
                    window.setNick = function(val) {
                        ...
                        HideOverlay();
                        nickName = $('#nick').val();
                        //nickNameSent = $('#nick').val()
                        //console.log('nickNameSent: ', nickNameSent)
